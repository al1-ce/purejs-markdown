...
const MD_FBASE = {
    ...
    NONE: function (html, match, regex, rstr, group) {
        return html;
    }
}
...
const MD_FCUSTOM = {
    HEADER: function (html, match, regex, rstr, group) {
        ...
        return html.replace(match[0], `<h${h}>${
            match[2]
        }</h${h}>`);
    },
    HEADER_ALT: function (html, match, regex, rstr, group) {
        ...
        let h = match[2] == '=' ? 1 : 2;
        let c = '';
        if (MD_OPTIONS.CENTER_ALT_HEADERS) 
            c = ` style="text-align: center;"`
        
        return html.replace(match[0], `<h${h}${c}>${
            match[1]
        }</h${h}>`);
    },
    LINK: function (html, match, regex, rstr, group) {

        let title = typeof match[4] == 'undefined' ? '' : `title="${
            match[4]
        }"`;
        if (match[0].charAt(0) == '!') {
            return html.replace(match[0], `<img src="${
                match[2]
            }" alt="${
                match[1]
            }" ${title}}></img>`);
        } else {
            return html.replace(match[0], `<a href="${
                match[2]
            }" ${title}}>${
                match[1]
            }</a>`);
    ...
    LINK_TO_FOOTER: function (html, match, regex, rstr, group) {
        ...
        if (!(match[2] in MD_FOOTNOTES.FOOTLINK)) {
            return html;
        }
        let href = MD_FOOTNOTES.FOOTLINK[match[2]].link;
        let title = typeof MD_FOOTNOTES.FOOTLINK[match[2]].title == 'undefined' ? '' : `title="${
            MD_FOOTNOTES.FOOTLINK[match[2]].title
        }"`;
        if (match[0].charAt(0) == '!') {
            return html.replace(match[0], `<img src="${href}" alt="${
                match[2]
            }" ${title}></img>`);
        } else {
            return html.replace(match[0], `<a href="${href}" ${title}>${
                match[1]
            }</a>`);
        }
    },
    ...
    CODE: function (html, match, regex, rstr, group) {
        return html.replace(match[0], `</p><pre><code>${
            match[group]
        }</code></pre><p>`);
    },
    ...
    FOOTNOTE_DEF: function (html, match, regex, rstr, group) {
        ...
        if (Object.keys(MD_FOOTNOTES.FOOTNOTE).length == 0) 
            html = `${html}<hr id='footer'>`;
        
        if (!(match[1] in MD_FOOTNOTES.FOOTNOTE)) {
            MD_FOOTNOTES.FOOTNOTE[match[1]] = {
                id: Object.keys(MD_FOOTNOTES.FOOTNOTE).length + 1,
                num: 0
            };
        }
        let f = MD_FOOTNOTES.FOOTNOTE[match[1]].id;
        return `${
            html.replace(match[0], ``)
        }<p id="${
            match[1]
        }">${f}. ${
            match[2]
    ...
    FOOTNOTE_INLINE: function (html, match, regex, rstr, group) {
        ...
        let id = `inlinefootnote-${
            Object.keys(MD_FOOTNOTES.FOOTNOTE).length + 1
        }`;
        if (Object.keys(MD_FOOTNOTES.FOOTNOTE).length == 0) 
            html = `${html}<hr id='footer'>`;
        
        MD_FOOTNOTES.FOOTNOTE[id] = {
            id: Object.keys(MD_FOOTNOTES.FOOTNOTE).length + 1,
            num: 0
        };
    ...
    TABLE: function (html, match, regex, rstr, group) {
        ...
        let align = [];
        let al = cols[1][0].split('|');
        al = al.slice(1, al.length - 1);
        ...
        let hd = cols[0][0].split('|');
        hd = hd.slice(1, hd.length - 1);
        tbl += '<tr>';
        Array.prototype.forEach.call(hd, function (e, i) {
            tbl += `<th style="text-align:${
                align[i]
            };">${e}</th>`
        });
        tbl += '</tr>';
        for (let i = 2; i < cols.length; i++) {
            let col = cols[i][0].split('|');
            col = col.slice(1, col.length - 1);
            tbl += '<tr>';
            Array.prototype.forEach.call(col, function (e, i) {
                tbl += `<td style="text-align:${
                    align[i]
                };">${e}</td>`
            });
            tbl += '</tr>';
...
const MD_REGEX_EARLY = {
    ...
    link_foot_def: {
        ...
        rstr: '',
        group: 1,
        func: MD_FCUSTOM.FOOTER_LINK
    },
    link_foot: {
        regex: /!?\[(.*?)\]\[(.*?)\]/gm,
        rstr: '',
        group: 1,
        func: MD_FCUSTOM.LINK_TO_FOOTER
    },
    ...
    footnote_inline: {
        ...
        rstr: '',
        group: 1,
        func: MD_FCUSTOM.FOOTNOTE_INLINE
    }
}
...
const MD_REGEX_LATE = {
    ...
    double_br_second_sweep: {
        regex: /(<br> *[\n\r]?<br>)+/gm,
        rstr: '<br>',
        group: 1,
        func: MD_FBASE.REPLACE
    },
    rm_br_before_closetag: {
        regex: /<br>\s*?[\r\n]?\S?(<\/.*?>)/gm,
        rstr: '',
        group: 1,
        func: MD_FBASE.KEEP_GROUP
    },
    rm_br_after_opening: {
        regex: /(<[^<\/]*?>) *?[\n\r]?<br>/gm,
        rstr: '',
